...
CREATE TABLE IF NOT EXISTS `UVisa2017`.`Carrera` (
  ...
  `creditos` INT(11) GENERATED ALWAYS AS (((`num_cred_opt` + `num_cred_tran`) + `num_cred_obl`)) VIRTUAL,
...
-- -----------------------------------------------------
-- Table `UVisa2017`.`Roles`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `UVisa2017`.`Roles` ;

CREATE TABLE IF NOT EXISTS `UVisa2017`.`Roles` (
  `NIF` VARCHAR(9) NOT NULL,
  `tipo_user` ENUM('ADMIN', 'ALUMNO', 'PROFESOR') NOT NULL DEFAULT 'ALUMNO',
  PRIMARY KEY (`NIF`),
...
CREATE TABLE IF NOT EXISTS `UVisa2017`.`Usuario` (
  ...
  CONSTRAINT `fk_Usuario_1`
    FOREIGN KEY (`NIF`)
    REFERENCES `UVisa2017`.`Roles` (`NIF`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `UVisa2017`.`Grupo` (
  ...
  `actas` TINYINT(1) NULL DEFAULT NULL,
