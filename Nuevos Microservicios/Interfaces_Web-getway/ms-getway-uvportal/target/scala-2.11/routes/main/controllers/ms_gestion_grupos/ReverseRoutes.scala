...
package controllers.ms_gestion_grupos {
  ...
  class ReverseGestionGruposDeAsignaturas_ApiController(_prefix: => String) {
    ...
    // @LINE:60
    def asignaturasMatriculablesByAlumnoNumeroExpedienteGet(numeroExpediente:Integer): Call = {
      import ReverseRouteContext.empty
      Call("GET", _prefix + { _defaultPrefix } + "asignaturasMatriculablesByAlumno/" + implicitly[PathBindable[Integer]].unbind("numeroExpediente", numeroExpediente))
    }
